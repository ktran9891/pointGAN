...
class PartDataset(data.Dataset):
    def __init__(self, root, npoints=2500, classification=False,
                 class_choice=None, train=True, parts_also=False,
                 shape_comp=False):
        ...
        if not self.classification:
            for i in range(len(self.datapath)/50):
                len_ = len(np.unique(np.loadtxt(self.datapath[i][-1]).astype(np.uint8)))    # noqa: E501
                if len_ > self.num_seg_classes:
                    self.num_seg_classes = len_
    ...
    def __getitem__(self, index):
        ...
        if self.parts_also:
            for j in np.unique(seg):
                ...
                choice2 = np.random.choice(part.shape[0],
                                           self.npoints/4,
                                           replace=True)
                part = part[choice2, :]
        ...
        if self.shape_comp:
            ...
            choice2 = np.random.choice(incomp.shape[0],
                                       4 * self.npoints/5,
                                       replace=True)
...
if __name__ == '__main__':
    ...
    d = PartPosDataset(root='shapenetcore_partanno_segmentation_benchmark_v0',
                       class_choice=['Chair'])
    print(len(d))
    ps, centers, seg = d[0]
    print(ps.size(), ps.type(), seg.size(), seg.type())
